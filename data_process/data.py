...
class SingleGraphDataset(InMemoryDataset, ABC):
    ...
    # for wikics and arxiv we use the text feautures with LLM encoder
    # cora, pubmed use_pca = True
    def __init__(self, cfg, 
                       name, 
                       transform=None,   
                       pre_transform=None, 
                       llm_encoder = SentenceEncoder,  # For Cora, Pubmed, set False , for arxiv, wikics set True
                       load_text = False): #  For Cora, Pubmed, set False , for arxiv, wikics set True
        self.cfg = cfg
        self.ds_name = name
    ...
    def add_text_emb(self, data_list, text_emb):
        data_list[0].node_text_feat = text_emb[0] # 节点特征的text feature
        data_list[0].edge_text_feat = text_emb[1]  # 对于cora数据集，边只有一个text句子
        # data_list[0].noi_node_text_feat = text_emb[2]
        data_list[0].class_node_text_feat = text_emb[3]  # 类别的text feature, 7个类别的包含prompt的label text
    ...
    def _build_adj(edge_index, num_nodes):
        ...
        # Build scipy csr adjacency for compatibility with existing code
    ...
    def gen_data(self):
        ds_alias = self.cfg['_ds_meta_data'][self.ds_name].split(", ")[1]  # SingleTextGraph.WikiCS
        components = ds_alias.split(".")  # [SingleTextGraph, WikiCS]
        parent, child = components  # parent = SingleTextGraph, child = WikiCS
        if self.ds_name == 'wikics':
            # root = datasets/ofa/SingleTextGraph.WikiCS
            # torch_geometric.datasets.wikics.WikiCS
            dataset =  WikiCS(root = self.root)
            # datasets/ofa/SingleTextGraph.WikiCS/metadata.json
            ...
            # datasets/ofa/SingleTextGraph.arxiv
            ...
            # root = root = datasets/ofa/SingleTextGraph.arxiv
...
class EpisodeDataset(Dataset):
    """
    Generates n episodes per graph per epoch
    每个图n个， 共nM个 m-way, k-shot, t-query 的tasks
    ...
    def __init__(self, data: Data, k: int, m: int, n: int, t: int = 1):  # m-way, k-shot, t-query
        ...
        self.M = int(data.ptr.numel() - 1)  # number of graphs
        self.batch = data.batch
        self.y = data.y
    
    def __len__(self):
        return self.M * self.n  # M 个图， 每个图n个task， 所以一个epoch共有 nM 个task

    def _sample_episode(self, i):  # for graph i
        node_idx = (self.batch == i).nonzero(as_tuple = False).view(-1)  # node idx 
        labels = self.y[node_idx]  # labels of graph i
        classes = labels.unique()

        m = min(self.m, classes.numel())
        picked = classes[torch.randperm(classes.numel())[:m]] # 随机采样m个classes作为一个task的m-way
        support, query = [],[]
        for c in picked:
            idx_c = node_idx[labels == c] # 所有属于c的节点idx
            idx_c = idx_c[torch.randperm(idx_c.size(0))] 
            k = min(self.k, idx_c.size(0)//2) # k shot for class c
            support.append(idx_c[:k])
            query.append(idx_c[k:k+self.t])  # 1 query
        return {
            'graph': i,
            'classes': picked,   # (m, )
    ...
    def __getitem__(self, idx):  
        graph_id = idx // self.n   # 每个graph要有n个task， 在一个batch中
